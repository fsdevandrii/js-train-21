...
class Musician {
  ...
  #name; // Об'являємо приватне поле #name
  #instrument; // Об'являємо приватне поле #instrument
...
class Guitarist extends Musician {
  ...
  constructor(name, instrument, band) {
    super(name, instrument); // виклик конструктора батьківського класу з двома параметрами name та instrument
    this.#band = band; // присвоєння вхідного значення band до приватного поля #band
  }

  get band() {
    return this.#band; // гетер для приватного поля #band, повертає значення приватного поля #band
  }

  set band(newBand) {
    this.#band = newBand; // сетер для приватного поля #band, присвоює нове значення приватному полю #band
  }
...
// Додавання сетера band до класу Musician
Object.defineProperty(Musician.prototype, "band", {
  set: function (newBand) {
    this.band = newBand; // this.band = newBand
  },
});
...
class Band {
  ...
  #name; // Об'являємо приватне поле #name
  #members; // Об'являємо приватне поле #members
  /*
  ...
  constructor(name, members) {
    this.#name = name; // Ініціалізуємо поле #name з вхідним значенням name
    this.#members = members; // Ініціалізуємо поле #members з вхідним значенням members
  }
  // Створюємо getter для #name, що повертає приватну властивість #name
  get name() {
    return this.#name; // Створюємо getter для #name, що повертає приватне поле #name
  }
  // Створюємо getter для #members, що повертає приватну властивість #members

  get members() {
    return this.#members; // Створюємо getter для #members, що повертає приватне поле #members
  }
  ...
  addMember(newMember) {
    if (newMember instanceof Musician) {
      ...
      newMember.band = this.#name; // Встановлюємо гурт для нового учасника за допомогою сетера band
      this.#members.push(newMember); // Додаємо нового учасника до приватного поля #members, яке є масивом
    } else {
      console.log("Новий учасник повинен бути екземпляром класу Musician"); // Якщо ні, виводимо в консоль повідомлення
    }
  }
...
class Performance {
  ...
  #location; // Об'являємо приватне поле #location
  #date; // Об'являємо приватне поле #date

  // Створюємо конструктор з трьома вхідними параметрами: #band, #location та #date
  constructor(band, location, date) {
    this.#band = band; // Ініціалізуємо поле #band з вхідним значенням band
    this.#location = location; // Ініціалізуємо поле #location з вхідним значенням location
    this.#date = date; // Ініціалізуємо поле #date з вхідним значенням date
  }
...
class Concert extends Performance {
  #ticketPrice; // Об'являємо приватне поле #ticketPrice;
  ...
  constructor(band, location, date, ticketPrice) {
    super(band, location, date); // використання super для виклику конструктора базового класу з трьома параметрами

    this.#ticketPrice = ticketPrice; // Ініціалізуємо поле #ticketPrice з вхідним значенням ticketPrice
  }
 ...
 */
class SongWriter {
  #songs; // Об'являємо приватне поле #songs

  // Створюємо конструктор з одним вхідним параметром: #songs
  constructor(songs) {
    this.#songs = songs; // Ініціалізуємо поле #songs з вхідним масивом songs
  }
  ...
  addSong(newSong) {
    this.#songs.push(newSong); // Додаємо нову пісню до масиву #songs
  }
...
class LeadSinger extends Vocalist {
  ...
  // super(name, band);
  constructor(name, band) {
    super(name, band); // Викликаємо конструктор батьківського класу за допомогою super з параметрами name, band
  }
}
